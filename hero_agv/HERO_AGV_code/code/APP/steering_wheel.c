...
double adjustTarget(double target, double actual)
{
	double theta = target - actual; // 计算theta，即target与actual的差值

	// 检查|theta|是否大于π/2
	if (fabs(theta) > HALF_PROTOCOL_POSITION_LSBS / 2)
	{
		// 需要调整target
		if (theta > 0)
		{
			// 如果theta是正数且大于π/2，从target中减去π（相当于给theta加上-π）
			target -= HALF_PROTOCOL_POSITION_LSBS;

			// 确保target在[0, 2π]范围内
			if (target < 0)
			{
				target += 2 * HALF_PROTOCOL_POSITION_LSBS; // 如果小于0，则加上2π
			}
		}
		else
		{
			// 如果theta是负数且绝对值大于π/2，给target加上π
			target += HALF_PROTOCOL_POSITION_LSBS;

			if(target > 2*HALF_PROTOCOL_POSITION_LSBS)
			{
				target -= 2* HALF_PROTOCOL_POSITION_LSBS;
			}
		}
	}
	// 如果|theta| <= π/2，则不需要调整target
...
{	
	...
	steering_wheel->parameter.arc_optimization = ENABLE_MINOR_DEG_OPTIMIZEATION;
	if (Steering_CheckHandleLegitimacy(steering_wheel) == STEERING_WHEEL_OK) 
	{	
		int32_t temp_err;
		#ifdef _OLD_CODE
		...
		#ifdef _NEW_CODE
		//舵轮轮组优化
		float theta_rad = (float)(steering_wheel->directive_part.command.protocol_position - steering_wheel->directive_part.status.protocol_position)/(float)PROTOCOL_POSITION_LSBS*2.0f*PI;
		temp_err = steering_wheel->motion_part.command.protocol_speed*cos(theta_rad) - steering_wheel->motion_part.status.protocol_speed;
		steering_wheel->motion_part.motor.command.torque = PID_Controller(&steering_wheel->motion_part.motor.PID_Handles.velocity_loop_handle, temp_err);

		temp_err = 	(int32_t)adjustTarget(steering_wheel->directive_part.command.protocol_position,steering_wheel->directive_part.status.protocol_position);
		#endif
		//更改
		 if(abs(temp_err)<200)
...
STEERING_WHEEL_RETURN_T Steering_Wheel_SetEnableStatus(steering_wheel_t *steering_wheel,uint8_t enable_status)
{
	steering_wheel->command.chassis_status = enable_status;
}
