...
steering_communication_pack_t steering_communication_SET_VELOCITY_VECTOR_handler(steering_wheel_t *steering, steering_communication_pack_t rx_pack)
{
	...
	uint16_t tmp_angle;
	uint16_t tmp_speed;
	uint8_t tmp_chassis_status;
	memcpy(&tmp_angle,rx_pack.data1,2);
	memcpy(&tmp_speed,rx_pack.data1+2,2);
	memcpy(&tmp_chassis_status,rx_pack.data1+4,1);
	float target_angle = Math_Int_To_Float(tmp_angle,0,0xFFFF,0.0f,8191.0f);
	Steering_Wheel_SetProtocolPosition(steering,(uint16_t)target_angle);
	Steering_Wheel_SetProtocolSpeed(steering,(uint16_t)tmp_speed);
	Steering_Wheel_SetEnableStatus(steering,(uint8_t)tmp_chassis_status);
		return rx_pack;
}
/*
